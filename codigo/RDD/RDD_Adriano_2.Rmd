...
#Função RDD evitando erro de filtrar apenas 2020 
```{r}
Rdd20 <- function(df, x, y) {
  library(rdrobust)
  library(rlang)

  y_name <- as_label(enquo(y))

  # Filtra valores inválidos (NA / Inf / NaN)
  df <- df |>
    filter(
      is.finite({{x}}),
      is.finite({{y}}),
  ...
  # Checagem mínima de dados após o filtro: garante q tem uma qnt de municípios aceitável próximo ao cutoff e que y tem uma variabilidade- se y é praticamente constante, é impossível captar diferença> RDD dá erro
  if (nrow(df) < 30 || length(unique(df |> pull({{y}}))) < 5) {
    cat("\n====================\n")
    cat("Variável:", y_name, "- não tem dados suficientes. Pulando.\n")
    return(NULL)
  ...
  # Captura erros do rdrobust sem parar o map()
  regressao <- tryCatch(
    rdrobust(
      x = x_vals,
      y = y_vals,
    ...
    error = function(e) {
      cat("\n====================\n")
      cat("Erro no rdrobust para variável:", y_name, "\n")
      cat("Mensagem do erro:", e$message, "\n")
      return(NULL)
    }
  )

  if (is.null(regressao)) return(NULL)
...
#Teste robustez simsus- exatamente igual codigo original
```{r}
...
```{r}
# Escolha uma variável do SINAN (exemplo)
y_var_escolhida <- "y_sinan_mental_encaminhamento_caps"


# Monta matriz de controles (mesma lógica do seu exemplo)
controls <- model.matrix(
  ~ ano + cov_reeleicao + cov_candidato_partido + cov_candidato_educacao_agrupada +
    cov_candidato_idade + cov_municipio_proporcao_mulheres_pop + cov_municipio_pop_total,
  data = df_rdd_sinan
)


# sequência de bandwidths (mesmo padrão do seu teste)
bandwidths <- seq(0.05, 1, by = 0.05)
...
for (i in seq_along(bandwidths)) {
  ...
  rd_robust_result <- tryCatch(
    rdrobust(
      x = df_rdd_sinan$val_margem_vitoria_feminina,
      y = df_rdd_sinan[[y_var_escolhida]],
      covs = controls,
      cluster = df_rdd_sinan$cd_municipio_ibge_7,
      h = h,
      all = TRUE,
      kernel = "triangular"
    ),
...
results |>
  ...
  facet_grid(rows = vars(name), scales = "free_y") +
  labs(
    title = paste("Robustez RDD –", y_var_escolhida),
    x = "Bandwidth (h)",
    y = "Valor"
  )
...
# matriz de controles (mesmo padrão do seu exemplo)
controls <- model.matrix(
  ~ ano + cov_reeleicao + cov_candidato_partido + cov_candidato_educacao_agrupada +
    cov_candidato_idade + cov_municipio_proporcao_mulheres_pop + cov_municipio_pop_total,
  data = df_rdd_sinan
...
# sequência de bandwidths (igual ao seu teste)
bandwidths <- seq(0.05, 1, by = 0.05)
...
for (var in y_vars) {
  ...
  message("Rodando teste de robustez para variável: ", var)
  ...
  lista_resultados[[contador]] <- tibble(
    y_var    = var,
    bandwidth = bandwidths,
    beta      = betas,
    pvalue    = pvalues
...
# consolida tudo em um único dataset
resultados_robustez <- bind_rows(lista_resultados)

resultados_robustez |>
  pivot_longer(beta:pvalue) |>
  ggplot(aes(x = bandwidth, y = value)) +
  geom_line() +
  facet_grid(rows = vars(name), cols = vars(y_var), scales = "free_y") +
  theme_minimal() +
  labs(
    title = "Teste de Robustez – SINAN (RDD)",
    subtitle = "Coeficiente e p-value variando o bandwidth",
    x = "Bandwidth (h)",
    y = "Valor"
  )
